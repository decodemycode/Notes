        ...
        html +=
            `<div id = "${index}" class="noteCard my-2 mx-2 card" style= "border: 1px solid #ced4da; height:${element.h}; width:${element.w}; min-height:12rem; min-width:16rem; resize: both; overflow: auto;">
                <div class="card-body">
                    ...
                    <h5 class="card-title" style="margin-left: 1.5em; display:inline">Note: ${String(index + 1)}</h5> 
...
const myObserver = new ResizeObserver(entries => {
    entries.forEach((entry, index) => {
        ...
        notesobj = JSON.parse(notes);
        notesobj[parseInt(entry.target.id)].w = String(entry.contentRect.width) + "px";
        notesobj[parseInt(entry.target.id)].h = String(entry.contentRect.height) + "px";
        localStorage.setItem('notes', JSON.stringify(notesobj));
    });
});
